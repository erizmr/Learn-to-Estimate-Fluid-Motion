...
import numpy as np
import json
from fluidestimator.modeling.loss_functions import *
...
import datetime
import os
from fluidestimator.modeling.liteflownet import estimate, device
...
def train(model, optimizer, criterion, data_loader):
    ...
    for x, y in data_loader:
        ...
        real_timeEPE = real_EPE(output_forward[-1], y).item()
...
def train_model(model,
    ...
    if optimizer is None:
        ...
        optimizer = optimizer
    criterion_train = multiscale_unsupervised_error
    criterion_validate = real_EPE
    ...
    for epoch in range(epoch_trained, n_epochs):
        ...
        loss_records['AEE_train_loss'].append(train_loss_epe)
        loss_records['AEE_validate_loss'].append(validation_loss_epe)
        loss_records['accumulative_time'].append(total_time)
        ...
        if (epoch + 1) % 5 == 0:
            ...
            output_path = './'
            path = os.path.join(output_path, "loss_records.json")
            json.dump(loss_records, open(path, 'w'))
